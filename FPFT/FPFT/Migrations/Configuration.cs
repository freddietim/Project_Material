namespace FPFT.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<FPFT.DAL.FLFContext>
    {
        ...
        protected override void Seed(FPFT.DAL.FLFContext context)
        {
            ...
            foreach (Email e in emailadd)
            {
                var emailInDataBase = context.EmailAdd.Where(
                    s =>
                         s.User.ID == e.UserID &&
                         s.Item.ItemID == e.ItemID).SingleOrDefault();
